...
class Stitch:
    ...
    def prepare_lists(self):
        ...
        self.centerIdx = self.count // 2   # ✅ FIXED
    ...
    def rightshift(self):
        for each in self.right_list:
            ...
            dsize = (
                int(txyz[0]) + self.leftImage.shape[1],
    ...
    def mix_and_match(self, leftImage, warpedImage):
        ...
        for i in range(i1x):
            for j in range(i1y):
                try:
                    if (np.array_equal(leftImage[j, i], [0, 0, 0]) and
                        np.array_equal(warpedImage[j, i], [0, 0, 0])):
...
if __name__ == '__main__':
    ...
    print("done")
    cv2.imwrite("test1.jpg", s.leftImage)
    print("image written")
